...
async function searchSokmil(keyword) {
  try {
    ...
    const keywordPrompt = `あなたは非常に優秀な検索アシスタントです。あなたは非常に優秀なAV作品の検索エンジンです。以下の文章から検索に使うタイトルに含まれていそうな日本語の名詞または形容詞あるいは女優名を1~5つまで抽出し、さらに文章から類推されるAVのジャンルを3つ生成し、それら合計4~8つの単語を「女優名」「キーワード」の2つのカテゴリに分類してください。
文章: "${searchQuery}"
